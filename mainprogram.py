...
def scrape_property(chemical_name, property): #chatgpt寫，自己擴充成可以查詢各種property
...
try:
    setnumber = int(input("請輸入1234以選擇你想查詢的藥品性質："))
    ...
    if setnumber in [1, 2, 3]:
        ...
        property_searched = []
        print("請以小寫輸入想查詢的化合物性質，每行一個性質，輸入 'end' 結束：")
        while True:
            line = input()
            if line.lower() == "end":
                break
